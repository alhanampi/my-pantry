...
**Codebase integrity check (knip) — shipped**: added [knip](https://knip.dev) as a root devDependency (`npm run knip`), configured via `knip.json` with two workspaces — `.` (frontend `src/` + the Vercel `api/` entry point) and `backend` (its own `src/`, `scripts/`, `prisma/seed.ts`) — since `backend/` has its own separate `package.json`/`node_modules`, not an npm workspace of the root. `ignoreDependencies` on the `.` workspace covers `@clerk/backend`, `@prisma/client`, `cors`, `express`, `express-rate-limit`, `express-validator`, `helmet`, `@types/cors`, `@types/express`: these look unused from the root project's own import graph, but are real, intentional duplicates of `backend/package.json`'s own runtime deps — `api/index.ts` imports `backend/src/app` directly (crossing the workspace boundary), and Vercel builds that serverless function against the *root* `node_modules`, not `backend/`'s, so these need to be resolvable from root even though nothing in `src/`/`api/` imports them directly.

Findings from the first run, applied:
- `src/styles/theme.ts` deleted — a fully orphaned, pre-color-scheme-system MUI theme (single hardcoded palette) with zero imports anywhere; theme building now lives entirely in `src/styles/colorSchemes.ts` (`baseThemeOptions`/`createAppTheme()`). `docs/ui.md`'s "Theme file locations" updated to match.
- `dev-dist/registerSW.js` (a `vite-plugin-pwa` dev-mode build artifact) was accidentally committed — untracked via `git rm --cached` and `dev-dist/` added to `.gitignore`.
- `backend`'s `bcrypt`/`jsonwebtoken` (+ their `@types/*`) removed — leftover from before the Clerk migration, no longer imported anywhere.
- `workbox-precaching` (imported directly in `src/sw.ts`) was only resolving as a transitive dependency of `vite-plugin-pwa` — added as an explicit root devDependency pinned to the version already in use, so a future `vite-plugin-pwa` upgrade can't silently break the service worker build by de-hoisting it.
- A handful of exports used only within their own file (`nutrientValue`/`ingredientNames` in `backend/src/services/recipeSerializers.ts`, `useToggleFavorite` in `src/hooks/useFavorites.ts`, `PREDEFINED_RESTRICTIONS` in `DietaryChipPicker`) had the `export` keyword dropped — no external caller, no reason to keep them public.
- Two genuinely dead styled-components deleted: `ChatView.styles.ts`'s `SectionTitle` (ChatView actually reuses `RecipesView.styles`'s own `SectionTitle` via `RecipeDetailPanel`, per the Recipes tab entry above — this one was never imported) and `ShoppingItem.styles.ts`'s `DeleteButton` (superseded by a plain MUI `IconButton` inline, `IconButton`'s own import in that file removed too since nothing else used it).

Left as-is (expected, not bugs): `apiUpdateShoppingList` (`src/api/pantryApi.ts`) stays flagged — it's the intentional pre-wired stub for the not-yet-built list-rename UI, see "Multiple Shopping Lists" above; commented in place so this is clear on future knip runs. The seven "unused exported types" in `src/utils/types.ts` (`SortDirection`, `ModalContext`, `RecipeMeasure`, `RecipeNutrition`, `DeleteConversationDialogState`, `AuthUser`) plus `ChatComposerProps` were deliberately left exported — each is genuinely used, just structurally (embedded as a field's type inside another exported interface, or a component's conventionally-exported `Props` type) rather than imported by name elsewhere; stripping the `export` would add churn to a shared domain-types file for no real benefit.
...
**Navigation icons — resolved**: the tab icons (Header's desktop tabs and BottomNav's mobile nav — pantry/recipes/favorites/chat/shopping) previously mixed `react-icons/md`'s filled variants with one `@mui/icons-material` icon (`EmojiObjectsIcon`, for Chat), which never read as visually consistent despite a couple of rounds of size/weight tweaks. Fixed by moving all 5 to the `MdOutline*` variant of `react-icons/md` (`MdOutlineRestaurantMenu`, `MdOutlineFavorite`, `MdOutlineLightbulb`, `MdOutlineShoppingCart`) — same thin, uniform stroke across the board, no new icon family introduced (stays inside the already-documented `react-icons/md` convention, see `docs/ui.md`). `@mui/icons-material` was removed from `package.json` entirely — it had no other usage in the codebase.
  - **Follow-up**: the pantry tab (`MdOutlineStorefront`) still stood out even after the outline pass — Material's storefront glyph has an inherently busier design (awning stripes, doorway) than the simple line icons around it. Swapped to `MdOutlineInventory2` (a box glyph, arguably a better semantic fit for "pantry inventory" too) in both Header and BottomNav. BottomNav's tabs also dropped their text labels (`label` prop removed from `NavAction`, kept as `aria-label` for assistive tech) and set `showLabels={false}` on `StyledBottomNavigation` — besides the icon-only look requested, this also stops MUI from applying its selected-vs-unselected sizing difference, so all 5 icons render at the same size regardless of which tab is active.
  - **Follow-up 2**: Header's desktop tab strip (`DesktopTabs`) now makes the *selected* tab feel like part of the same sheet as the content below it, instead of an underline-indicator strip sitting on top of a separate page. `desktopTabSx` in `src/components/Header/index.tsx` gives the selected tab (`&.Mui-selected`) `background-color: var(--scheme-bg)` — the same background `MainContent` sits on — plus rounded top corners (`8px 8px 0 0`) and squared-off bottom corners, so it visually merges straight into the page instead of floating above it; the old colored underline indicator (`TabIndicatorProps`) was removed since the background match now carries the selection cue. Unselected tabs are unchanged (still sit on `--scheme-on-primary-tabs`, the translucent strip over the AppBar's primary color).
  - **Follow-up 3**: BottomNav (the mobile equivalent) has no adjoining content panel to merge into — it's a fixed bar floating at the bottom of the screen — so the Header's "merge with the page" trick doesn't apply as-is there. First attempt was a Material-3-style pill behind just the icon (padding directly on the `<svg>`, since react-icons renders it with no wrapper span to target) — reverted: with `CssBaseline`'s global `box-sizing: border-box`, padding on an element with a fixed `width`/`height` attribute (the icon's `size={24}`) shrinks its visible content instead of growing the box, so the icon visibly shrank when selected. Replaced with `NavAction`'s `&.Mui-selected` setting `background-color: var(--scheme-bg)` on the whole tab. Second attempt rounded the top corners (`border-radius: 12px 12px 0 0`) — also reverted, since a rounded chip reads as a separate floating box (the rounding shows a sliver of the bar's own background through the corners, breaking the "one continuous surface" look). Final version: no border-radius at all, a flush square-edged fill spanning the tab's full slot, so it reads as a straight continuation of the page background rather than a discrete highlighted box.
